...
#define GZP_BUILD_STAMP "R1"
#define GZP_SCHEMA      1
#define PREFIX          "GZP_"

input string  DataFile        = "GZP\\zones_gzp.json"; // Файл зон (MQL5/Files)
...
int OnInit()
  {
   IndicatorSetString(INDICATOR_SHORTNAME, "GZP Zones " + GZP_BUILD_STAMP);
...
datetime ParseIsoTime(string iso)
  {
   ...
   StringReplace(date, "-", ".");
   return(StringToTime(date + " " + clock));
  }
...
void DrawZones()
  {
   ...
   datetime rightEdge = lastBar + PeriodSeconds() * ZoneExtendBars;
   int firstVisible = (int)ChartGetInteger(0, CHART_FIRST_VISIBLE_BAR);
   datetime leftEdge = lastBar - PeriodSeconds() * MathMax(firstVisible, 1);
